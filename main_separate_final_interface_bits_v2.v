...
module a #(
    ...
    input  wire                   ena,
    input  wire                   wea,
    input  wire [BRAM_DEPTH-1:0] addra,
    input  wire [DATA_WIDTH-1:0]  dina,
    output reg  [DATA_WIDTH-1:0]  douta
);
    reg [DATA_WIDTH-1:0] mem [0:BRAM_DEPTH-1];
    reg [DATA_WIDTH-1:0] stage1;  // pipeline register
...
module outp #(
    ...
    input  wire                   enb,
    input  wire                   web,
    input  wire [BRAM_DEPTH-1:0] addrb,
    ...
    always @(posedge clka) begin
        if (ena) begin
            if (wea)
                mem[addra] <= dina;
            douta <= mem[addra];   // 1-cycle latency
        end
    end
    ...
    always @(posedge clkb) begin
        if (enb) begin
            if (web)
                mem[addrb] <= dinb;
            doutb <= mem[addrb];   // 1-cycle latency
        end
    end
endmodule
...
//    output reg done,
   output [interface_bits-1:0] out,
    ...
    localparam data_width = 2*base_mult + $clog2(bram_depth) + 1;
    localparam data_depth = 2*bram_depth-1;
    localparam outp_depth = 2*bram_depth-1;
...
//    reg [2*base_mult:0] mult_pipe; 
    ...
    generate
        if (num_clocks == 1) begin: base_case
            ...
            assign mid_x = mid;
            outp #(data_width, data_depth) final_out (
            .clka(clk),    // input wire clka
...
//                    start <= 1'b0;
        ...
        assign out = outf;
            ...
            a_out1 #(base_mult, outp_depth) a_out(
              .clka(clk),    // input wire clka
