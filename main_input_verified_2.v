...
module generic_multiplier #(parameter base_mult = 128, parameter interface_bits = 128, parameter bram_depth = 512)(
    ...
    input [2*interface_bits-1:0] x,
    input [2*interface_bits-1:0] y,
//    output reg done
    ...
    always @(posedge clk) begin
        if (reset == 1'b0 && wein == 1'b1) begin
            if(c == 0 && num_blocks == 1)begin
                addra00 <= c;
                dina0 <= x[interface_bits-1:0];
                addra10 <= c;
                dina1 <= 0;
                
                addrb00 <= c;
                dinb0 <= y[interface_bits-1:0];
                addrb10 <= c;
                dinb1 <= 0;
            ...
            if(c <= num_blocks/2-1 && num_blocks != 1) begin
                addra00 <= c;
                dina0 <= x[interface_bits-1:0];
                addra10 <= c;
                dina1 <= x[2*interface_bits-1:interface_bits];
                
                addrb00 <= c;
                dinb0 <= y[interface_bits-1:0];
                addrb10 <= c;
                dinb1 <= y[2*interface_bits-1:interface_bits];
