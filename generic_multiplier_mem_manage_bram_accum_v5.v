...
module a (
    ...
    input wire ena,
    input wire wea,
    input wire [2047:0] addra,
    input wire [127:0] dina,
    output reg [127:0] douta
);
    reg [127:0] mem [0:2047];  // 64K x 32-bit
...
module out (
    ...
    input wire ena,
    input wire wea,
    input wire [4094:0] addra,
    input wire [267:0] dina,
    output reg [267:0] douta,

    input wire clkb,
    input wire enb,
    input wire web,
    input wire [4094:0] addrb,
    input wire [267:0] dinb,
    output reg [267:0] doutb
);

    reg [267:0] mem [0:4094];  // 64K-depth dual-port 128-bit memory
...
//    output reg done
    ...
    always @(posedge clk) begin
        ...
        else if (count - 4 >= num_blocks*num_blocks) begin
