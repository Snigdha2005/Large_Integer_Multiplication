...
generate
    if (num_bits <= base_mult) begin : base_case
        ...
        always @(posedge clk) begin
            ...
            out <= (start == 1)?({{(2*num_bits - 2*sh){1'b0}}, out_p2_reg} << (2*fh)) + ({{(2*num_bits - 2*(sh+1)){1'b0}}, middle} << fh) + ({{(2*num_bits - 2*fh){1'b0}}, out_p1_reg}):0;
        end
    end
endgenerate
endmodule
...
module top_kartasuba #(parameter num_bits = 64, parameter base_mult = 32)(
    ...
    reg [31:0] out_idx = 0;
    always @(posedge clk) begin
        if (count >= 11 && out_idx < 2*num_bits) begin
            out <= outf[out_idx +: 32];
            out_idx <= out_idx + 32;
        end
    end
