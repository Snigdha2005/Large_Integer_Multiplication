module base_multiplier #(parameter N = 32)(input clk, input [N-1:0] x, input [N-1:0] y, output reg [2*N:0] out);
...
module four_bit_mult #(parameter base_mult = 2)(
    ...
    generate
        if (base_mult != 4) begin : gen_karatsuba
            ...
            base_multiplier #(2) p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            base_multiplier #(2) p3(.x(xr), .y(yr), .out(out3), .clk(clk));
...
module eight_bit_mult #(parameter base_mult = 4)(
    ...
    generate
        if (base_mult != 8) begin : gen_karatsuba
            ...
            four_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            four_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module sixteen_bit_mult #(parameter base_mult = 8)(
    ...
    generate
        if (base_mult != 16) begin : gen_karatsuba
            ...
            eight_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            eight_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module thirty_two_bit_mult #(parameter base_mult = 16)(
    ...
    generate
        if (base_mult != 32) begin : gen_karatsuba
            ...
            sixteen_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            sixteen_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module sixty_four_bit_mult #(parameter base_mult = 32)(
    ...
    generate
        if (base_mult != 64) begin : gen_karatsuba_64
            ...
            thirty_two_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            thirty_two_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module one_twenty_eight_bit_mult #(parameter base_mult = 64)(
    ...
    generate
        if (base_mult != 128) begin : gen_karatsuba_128
            ...
            sixty_four_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            sixty_four_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module two_fifty_six_bit_mult #(parameter base_mult = 128)(
    ...
    generate
        if (base_mult != 256) begin : gen_karatsuba_256
            ...
            one_twenty_eight_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            one_twenty_eight_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module five_twelve_bit_mult #(parameter base_mult = 256)(
    ...
    generate
        if (base_mult != 512) begin : gen_karatsuba_512
            ...
            two_fifty_six_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            two_fifty_six_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module one_zero_two_four_bit_mult #(parameter base_mult = 512)(
    ...
    generate
        if (base_mult != 1024) begin : gen_karatsuba_1024
            ...
            five_twelve_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            five_twelve_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module two_zero_four_eight_bit_mult #(parameter base_mult = 1024)(
    ...
    generate
        if (base_mult != 2048) begin : gen_karatsuba_2048
            ...
            one_zero_two_four_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            one_zero_two_four_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module four_zero_nine_six_bit_mult #(parameter base_mult = 32)(
    ...
    generate
        if (base_mult != 4096) begin : gen_karatsuba
            ...
            two_zero_four_eight_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            two_zero_four_eight_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module eight_one_two_two_bit_mult #(parameter base_mult = 4096)(
    ...
    generate
        if (base_mult != 8192) begin : gen_karatsuba_8192
            ...
            four_zero_nine_six_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            four_zero_nine_six_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module sixteen_three_eight_four_bit_mult #(parameter base_mult = 8192)(
    ...
    generate
        if (base_mult != 16384) begin : gen_karatsuba_16384
            ...
            eight_one_two_two_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            eight_one_two_two_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module thirty_two_seven_six_eight_bit_mult #(parameter base_mult = 16384)(
    ...
    generate
        if (base_mult != 32768) begin : gen_karatsuba_32768
            ...
            sixteen_three_eight_four_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            sixteen_three_eight_four_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module sixty_five_five_three_six_bit_mult #(parameter base_mult = 32768)(
    ...
    generate
        if (base_mult != 65536) begin : gen_karatsuba_65536
            ...
            thirty_two_seven_six_eight_bit_mult p1(.x(xl), .y(yl), .out(out1), .clk(clk));
            thirty_two_seven_six_eight_bit_mult p3(.x(xh), .y(yh), .out(out3), .clk(clk));
...
module karatsuba #(
    ...
    generate
        if (num_bits == 65536) begin
            sixty_five_five_three_six_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 32768) begin
            thirty_two_seven_six_eight_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 16384) begin
            sixteen_three_eight_four_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 8192) begin
            eight_one_two_two_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 4096) begin
            four_zero_nine_six_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 2048) begin
            two_zero_four_eight_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 1024) begin
            one_zero_two_four_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 512) begin
            five_twelve_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 256) begin
            two_fifty_six_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 128) begin
            one_twenty_eight_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 64) begin
            sixty_four_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 32) begin
            thirty_two_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 16) begin
            sixteen_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 8) begin
            eight_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 4) begin
            four_bit_mult #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else if (num_bits == 2) begin
            base_multiplier #(base_mult) dut (.x(x), .y(y), .out(out), .clk(clk));
        end
        else begin
            base_multiplier #(num_bits) dut (.x(x), .y(y), .out(out), .clk(clk));
            // assign out = 0;
        end
    endgenerate
    // initial begin
...
module vectorised_karatsuba #(parameter num_single_lanes = 14348907, parameter num_bits = 4, parameter N = 10, parameter num_lanes = 5, parameter base_mult = 512)( 
    ...
    int cycles = 0;
    int batch_no = 0;
    int total_batches = (N % num_lanes == 0)? N / num_lanes: N / num_lanes + 1;
    ...
    //     cycles = cycles + 1;
    //     if (cycles % 4 == 0)begin
    //         // batch_no = ((batch_no + 1) < total_batches)? batch_no + 1 : batch_no;
    //         batch_no = batch_no + 1;
    //     end
    //     else begin
    //         batch_no = batch_no;
    ...
    always @(posedge clk1) begin
        ...
        // end
        if ((cycles % num_lanes == 0) && (cycles != 0))begin
            // batch_no = ((batch_no + 1) < total_batches)? batch_no + 1 : batch_no;
            batch_no = batch_no + 1;
        end
        else begin
        ...
        if (((cycles % num_lanes) == 0) && (batch_no -1 < total_batches))begin
            for (lane = 0; lane < num_lanes; lane = lane + 1) begin
                idx = batch_no * num_lanes + lane;
                // $display("cycles =  %d, batch_no = %d, idx = %d", cycles, batch_no, idx);
                if (idx < N) begin
                    in1_reg[lane] = x[idx];
                    in2_reg[lane] = y[idx];
                    result[idx-num_lanes] = partial_out[lane];
                    // $display("result = %d, idx = %d", result[idx-5], idx);
                    // $display("batch_no %d, cycles %d", batch_no, cycles);
                end 
                else if (idx < N + (total_batches+1)*num_lanes) begin
                    result[idx-num_lanes] = partial_out[lane];
                    // $display("batch_no = %d, result = %d, idx = %d", batch_no, result[idx-5], idx);
                end
                else begin
                    in1_reg[lane] = 0;
                    in2_reg[lane] = 0;
        ...
        cycles = cycles + 1;
        // $display("batch_no %d, cycles %d, %d", batch_no, cycles, total_batches);
    end
