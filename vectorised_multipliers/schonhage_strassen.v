...
module schonhage_strassen #(parameter num_bits = 32, parameter n = 8)(
    ...
    extract_digits #(n, num_bits) ext1(x, a_digits);
    extract_digits #(n, num_bits) ext2(y, b_digits);

    ntt #(7340033, 5, 4404020, 1<<20, n) fwd1(1'b0, a_digits, a_ntt);
    ntt #(7340033, 5, 4404020, 1<<20, n) fwd2(1'b0, b_digits, b_ntt);

    point_wise_mult #(n, 7340033) pwm(a_ntt, b_ntt, c_ntt);

    ntt #(7340033, 5, 4404020, 1<<20, n) inv(1'b1, c_ntt, result);

    carry_prop #(n) carry_p(result, result1);

    reconstruction_result #(n, num_bits) reconst(result1, out);
...
module vectorised_schonhage #(parameter num_bits = 32, parameter N = 10, parameter num_lanes = 5, parameter n = 8)(
    ...
    // wire [2*num_bits-1:0] partial_out [0:num_lanes-1];

    // reg [num_bits-1:0] in1_reg [0:num_lanes-1];
    // reg [num_bits-1:0] in2_reg [0:num_lanes-1];
    // reg [2*num_bits-1:0] result [0:N-1];
